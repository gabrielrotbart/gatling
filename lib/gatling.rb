...
module Gatling
  class Comparison
    ...
    def matches?
      @cropped_image = @capture_element.capture
      if !@trainer_toggle
    ...
    def compare
      expected_image = @file_helper.load(@expected, :reference)

      diff_metric = @cropped_image.compare_channel(expected_image, Magick::MeanAbsoluteErrorMetric)
